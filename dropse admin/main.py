...
# om namah shivay
# jai shri ram
# Sutantra Dropse ADMIN balaji by RS Enterprises
...
def delete_emp():
  ...
  ans=tkinter.messagebox.askyesno("Remove Employee",f"Are you sure you want to remove {values[3]} {values[0]}\n Doing this will remove all his details permanently",icon="warning")
  if ans:
...
def employee():
  ...
  if ans:
    ...
    if not data.empty:
      ...
      return
    data=[name_,email_,phone_,com_id_,password_,phone_+"@"+"dropse"+name_[1:5],datetime.datetime.now().strftime("%-m/%-d/%y"),0,0]
...
def work_detail():
  ...
  workdata_late=workdata_.loc[(workdata_["STATUS"]==1) &(work_data["END"].apply(strip)<today)]
  ...
  # getting emp info
  emp_search_tag=emp_search.get().strip()
  if len(emp_search_tag)!=0:
    ...
    id_label.configure(text=emp_search_tag) 
    name_label_tag=emp_data.loc[emp_data["CID"]==emp_search_tag,"NAME"].values[0]#since index and datatype will also be printed if not done
    name_label.configure(text=name_label_tag) 
    username_label_tag=emp_data.loc[emp_data["CID"]==emp_search_tag,"USERNAME"].values[0]#since index and datatype will also be printed if not done
    username_label.configure(text=username_label_tag)
  else:
    pass
...
root.title("Sutantra Dropse ADMIN Balaji")
...
emp_search= customtkinter.CTkEntry(top_frame, placeholder_text="emp cid",width=250)
...
customtkinter.CTkLabel(right_tabs.tab("Details"),text="id",font=font).grid(row=6,column=0)
id_label=customtkinter.CTkLabel(right_tabs.tab("Details"),text="",font=font)
